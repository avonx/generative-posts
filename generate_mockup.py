...
def max_circular_crop(image):
    ...
    result = Image.composite(cropped, Image.new('RGBA', cropped.size, (0, 0, 0, 0)), mask)
    return result
...
def place_text(draw, textbox_position, textbox_size, text, font, emoji_font, fill=(255, 255, 255)):
    ...
    # テキストボックス内にテキストを配置
    draw_text(draw, textbox_position, text, font, emoji_font, max_width=textbox_size[0], fill=fill)

# コレは絶対HTMLで描画する仕様にしておいたほうがよかった。。。
def generate_instagram_mockup(circle_image_path, rectangle_image_path, name, situation, comment):
    ...
    cropped_rectangle = max_rectangular_crop(rectangle_image, rect_size[0]/rect_size[1])
    ...
    emoji_font = ImageFont.truetype("/System/Library/Fonts/Apple Color Emoji.ttc", 20) # TODO ローカルパス
    
    # 名前を描画
    place_text(draw, (345, 122), (300, 35), name, font, emoji_font, fill=(20,20,20))

    # situationを描画
    place_text(draw, (345, 145), (300, 35), situation, font, emoji_font, fill=(180,180,180))

    # コメントを描画
    place_text(draw, (280, 850), (500, 120), comment, font, emoji_font, fill=(20,20,20))
...
if __name__ == '__main__':
    ...
    generated_mockup = generate_instagram_mockup(circle_image_path, rectangle_image_path, name, situation, comment)
