...
def generate_multimodal_post(
    name, looks, characteristics, situation, reference_image_path, without_person=False
):
    ...
    prompt_for_sd = generate_prompt_for_sd(
        looks=looks, situation=situation, without_person=without_person
    )
    ...
    situation_with_layout = (
        situation + f"次のような構図の画像を撮ってみた。「{prompt_for_sd}」"
    )  # situationは、引き継ぐが、撮った写真の構図を新たに加えておく。
    comment = generate_prompt_for_comment(
        name, looks, characteristics, situation_with_layout
    )
